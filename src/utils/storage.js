  ...
  try {
    ...
    Object.keys(allMessages).forEach(characterId => {
      ...
      if (messages && Array.isArray(messages)) {
        ...
        messages.forEach(message => {
          ...
          // 检查用户消息的头像
          if ((message.sender === 'user' || message.role === 'user') && currentAvatar !== targetAvatar) {
            // 存储实际的头像URL
    ...
    if (hasChanges) {
      ...
      try {
        ...
        console.error('保存到localStorage失败，尝试sessionStorage:', localError);
      }
      
      // 同时保存到sessionStorage作为备份
      try {
        sessionStorage.setItem(`session_${STORAGE_KEYS.CHAT_MESSAGES}`, JSON.stringify(allMessages));
...
export const saveUserInfo = (userInfo) => {
  try {
    ...
    // 保存到localStorage
    localStorage.setItem(STORAGE_KEYS.USER_INFO, JSON.stringify(processedUserInfo));
    // 同时保存到sessionStorage作为备份
    sessionStorage.setItem(`session_${STORAGE_KEYS.USER_INFO}`, JSON.stringify(processedUserInfo));
    
    // 使用真实头像URL进行比较，确保准确检测头像变化
    if (oldAvatar !== newAvatar && newAvatar) {
      // 静默更新历史消息中的头像
      const updateResult = updateUserAvatarInChatMessages(processedUserInfo.avatar);
    ...
    try {
      ...
      sessionStorage.setItem(`session_${STORAGE_KEYS.USER_INFO}`, JSON.stringify(compressedUserInfo));
      ...
      if (oldAvatar !== newAvatar && newAvatar) {
        ...
        const updateResult = updateUserAvatarInChatMessages(compressedUserInfo.avatar);
        
        if (!updateResult) {
          console.warn('首次更新头像失败，尝试使用真实头像URL再次更新（备选存储）');
          // 如果更新失败，尝试使用真实头像URL再次更新
          updateUserAvatarInChatMessages(newAvatar);
        }
      }
...
const processUserInfoBeforeSave = (userInfo, forceCompression = false) => {
  ...
  if (processedInfo.avatar && processedInfo.avatar.startsWith('data:image/')) {
    ...
    if (avatarSizeInKB > MAX_SIZE_IN_KB || forceCompression) {
      // 使用默认头像替代过大的头像
      processedInfo.avatar = 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80';
    }
  }
...
export const saveFavorites = (favorites) => {
  try {
    ...
    // 同时保存到sessionStorage作为备份
    sessionStorage.setItem(`session_${STORAGE_KEYS.FAVORITES}`, JSON.stringify(favorites));
  } catch (error) {
...
export const saveChatHistory = (history) => {
  try {
    ...
    // 同时保存到sessionStorage作为备份
    sessionStorage.setItem(`session_${STORAGE_KEYS.CHAT_HISTORY}`, JSON.stringify(history));
  } catch (error) {
...
export const getCustomAIs = () => {
  try {
    ...
    const useSessionStorage = localStorage.getItem(`${STORAGE_KEYS.CUSTOM_AIS}_use_session`) === 'true';
    ...
    if (useSessionStorage) {
      ...
      if (sessionRaw) {
        ...
        console.log(`从sessionStorage加载自定义AI列表（主要存储），共${customAIs.length}个AI`);
        return customAIs;
      }
    }
...
const processObjectWithAvatar = (obj) => {
  ...
  if (processedObj.avatar && processedObj.avatar.startsWith('data:image/')) {
    ...
    if (avatarSizeInKB > 100) { // 大于100KB的头像需要特殊处理
      // 创建一个唯一的键名
      const avatarKey = `temp_avatar_${obj.id || Date.now()}`;
      ...
      try {
        ...
        console.warn('无法保存头像到sessionStorage，保留原始头像数据:', error);
        // 即使sessionStorage保存失败，也不使用默认头像，保留用户自定义头像
      }
    }
  }
...
export const saveCustomAIs = (customAIs) => {
  try {
    // 先检查localStorage是否有足够空间
    // 创建一个测试项来验证localStorage容量
    ...
    // 检查数据大小是否可能超过localStorage限制
    if (dataToSave.length > 5 * 1024 * 1024) { // 5MB 是localStorage的常见限制
      console.warn('数据超过5MB，可能无法保存到localStorage');
      // 自动切换到sessionStorage
    ...
    // 同时保存到sessionStorage作为备份
    sessionStorage.setItem(`session_${STORAGE_KEYS.CUSTOM_AIS}`, dataToSave);
    return true;
  } catch (error) {
    console.error('保存到localStorage失败:', error);
    // 尝试使用sessionStorage作为主要存储
    try {
      const dataToSave = JSON.stringify(customAIs);
      sessionStorage.setItem(`session_${STORAGE_KEYS.CUSTOM_AIS}`, dataToSave);
      
      // 设置标志，指示下次加载时使用sessionStorage作为主要存储
      try {
        localStorage.setItem(`${STORAGE_KEYS.CUSTOM_AIS}_use_session`, 'true');
      } catch (e) {
      ...
      try {
        ...
        if (customAIs.length > 0) {
          ...
          const compressedData = JSON.stringify([lastAI]);
          sessionStorage.setItem(`${STORAGE_KEYS.CUSTOM_AIS}_last`, compressedData);
          console.log('已保存最近一个AI到sessionStorage');
          return true;
        }
      } catch (lastError) {
...
export const getChatMessages = (characterId) => {
  try {
    ...
    if (sessionRaw) {
      ...
      if (characterId !== 'all') {
        try {
          const localStorageRaw = localStorage.getItem(STORAGE_KEYS.CHAT_MESSAGES);
          const localStorageMessages = localStorageRaw ? JSON.parse(localStorageRaw) : {};
          localStorageMessages[characterId] = messages;
          localStorage.setItem(STORAGE_KEYS.CHAT_MESSAGES, JSON.stringify(localStorageMessages));
        } catch (syncError) {
